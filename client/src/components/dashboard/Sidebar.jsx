...
const Sidebar = () => {
  ...
  const navItems = [
    ...
    { icon: <Bot size={20} />, label: 'Generate Interview' },
    { icon: <FileText size={20} />, label: 'My Reports' },
    { icon: <Upload size={20} />, label: 'Resume Upload' },
    ...
    >
      <div>
        ...
        <div className="flex items-center gap-3 px-4 mb-12">
          ...
          <div>
            <h1 className="font-space text-lg font-bold text-white tracking-tight">Kinetic AI</h1>
            <p className="text-[10px] uppercase text-[#c0c1ff] opacity-70 tracking-widest font-semibold">Observatory</p>
          </div>
        </div>
        ...
        <nav className="flex flex-col gap-2">
          {navItems.map((item, idx) => (
            ...
            >
              <span className={`${item.active ? 'text-[#5de6ff] ai-glow-text' : ''}`}>
                {item.icon}
              </span>
